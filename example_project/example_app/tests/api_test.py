...
class ExampleModelAPITests(APITestCase):
    ...
    def test_writable_db_dict_list_list(self):
        url = reverse("example_app:translatable_writable_db_dict")
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertDictEqual(response.data[0]["name"], {"en": "Hello", "es": "Hola"})

    def test_writable_db_dict_retrieve(self):
        url = reverse("example_app:translatable_writable_db_dict", kwargs={"pk": 1})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertDictEqual(response.data["name"], {"en": "Hello", "es": "Hola"})

    def test_writable_db_dict_list_create(self):
        url = reverse("example_app:translatable_writable_db_dict")
        data = {
    ...
    def test_writable_db_dict_list_update(self):
        url = reverse("example_app:translatable_writable_db_dict", kwargs={"pk": 1})
        data = {
            "name": {"es": "Adiós"},
        }
